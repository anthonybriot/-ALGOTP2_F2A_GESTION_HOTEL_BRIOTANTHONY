...
Create table TJ_COUT (
		CHB_ID   integer not null,
		TRF_CHB_PRIX   real not null,
		primary key (CHB_ID),
...
Create table TJ_OCCUPATION	(
		CHB_ID integer not null,
		CHB_PLN_CLI_NB_PERS smallint not null,
		CHB_PLN_CU_RESERVE integer (1) not null,
		CHB_PLN_CU_OCCUPE integer(1) not null,
		primary key(CHB_ID),
		foreign key (CHB_ID) references T_CHAMBRE);
...
Create table T_TYPE	(
		TYPE_CODE char(8) not null,
		TYPE_LIBELLE char(32) not null,
